...
class CarFactory {
  ...
  //예(2) 인스턴스를 오직 한개만 생성해야 할 경우
  // 인스턴스를 여러개 생성할 필요가 없는 경우에 생성자를 private 로 막는다.
  // => getInstance() 스태틱 메서드를 통해 인스턴스를 한개만 만들어 사용한다.
...
public class Exam0222 {
  ...
  public static void main(String[] args) {
    ...
    // 자동차 공장 객체를 먼저 만든다.
    //    CarFactory factory = new CarFactory(); // 컴파일 오류. 이유는 생성자가 private이라서
    ...
    // 생성자가 private 접근으로 막혀있어 new 연산자를 이용하지 않고 스태틱 메서드를 호출해서 인스턴스를
    //    생성하는 경우 다음 둘 중 하나다.
