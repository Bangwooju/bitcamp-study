...
public class App {
  ...
  // Menu 추상 클래스를 상속받아 PMS 시스템에 맞게 기능을 추가한다.
  class MenuItem extends Menu {
    ...
    public void execute() {
      ...
      // inner 클래스는 바깥 클래스의 인스턴스를 내부 필드로 갖고있기 때문에
    ...
    public MenuItem(/*App outer, */ String title, String menuId) {
      this(title, ENABLE_ALL, menuId); // 다른 생성자와 코드가 중복된다면 다른 생성자를 호출한다.
      //this.$0 = outer; 자동으로 생긴다고
    }
  ...
  Menu createMenuItem() {
    ...
    boardMenu.add(new MenuItem("검색", Menu.ENABLE_LOGIN,"/board/search"));
