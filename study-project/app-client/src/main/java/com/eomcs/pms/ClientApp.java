...
public class ClientApp {
  ...
  public static void main(String[] args) throws Exception {
    ...
    while(true) {
      ...
      if(input.equals("/board/add")) {
        ...
        out.println(input);
        out.flush(); // 서버에게 메세지를 보내라는 명령어

        String result = in.readLine(); // 서버에서 한 줄의 문자열을 보낼 때 까지 기다린다.
  ...
  private static void addBoard(PrintWriter out , BufferedReader in) throws Exception {
    ...
    board.setRegisteredDate(Date.valueOf("2021-01-01"));
    Member m = new Member();
